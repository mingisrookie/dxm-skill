...
发布信息以仓库 [`VERSION`](VERSION)、tag 和 [GitHub Releases](https://github.com/mingisrookie/dxm-skill/releases) 为准。Skill 路径：`skills/dxm` · 核心脚本：`skills/dxm/scripts/scaffold_dxm.py`
...
| 更新 | 作用 |
| --- | --- |
| GitHub Actions CI | 在 Ubuntu 和 Windows 上自动跑 unittest 与 packaged `--self-test`。 |
| `--self-test` 收敛 | 只做安装完整性冒烟；模板内容级断言统一放在 `tests/`。 |
| Trellis dry-run 补全 | fresh 项目也会提示 `trellis init` 成功后要追加的 DXM/Trellis 安全写入。 |
| 文件结构降噪 | 跳过常见 venv、IDE、pytest/tox/mypy/ruff 缓存目录；`config.json` 不再无条件标敏感。 |
