...
[快速使用](#快速使用) · [v1.0.1 更新](#v101-更新重点) · [DXM 工作流](#dxm-工作流) · [Trellis 路由](#dxm--trellis) · [生成文件](#生成文件)
...
当前发布版：[`v1.0.1`](https://github.com/mingisrookie/dxm-skill/releases/tag/v1.0.1) · Skill 路径：`skills/dxm` · 核心脚本：`skills/dxm/scripts/scaffold_dxm.py`
...
| 更新 | 作用 |
| --- | --- |
| 需求询问先第一性原理 | 每次需求澄清先判断真实目标、硬约束、本地证据和未知阻塞。 |
| grill 调用前先质疑 | `grilling` / `grill-with-docs` / `grill-me` 不只收集答案，还要质疑隐藏假设、伪约束、过度方案和实现偏置。 |
| Trellis 完成后对抗性检查 | 每次 Trellis 任务 finish / handoff 前先做对抗性检查，阻断问题回到 implement/check。 |
| 模板和自测同步 | 生成的 `AGENTS.md`、项目规范、Trellis 管理块和 `--self-test` 都带上这些规则。 |

### v1.0.0 更新重点

| 更新 | 作用 |
| --- | --- |
| 基于真实开发蒸馏发布纪律 | 把最近实际使用中暴露的 release 漏项沉淀进 DXM 规则。 |
| 发布不止 push main | 明确版本号、`CHANGELOG.md`、tag、GitHub Release、Latest、中文更新日志、对比链接和验证证据必须同步。 |
| 生成文档内置 release checklist | 新项目的 `开发者AI开发与PR提交流程.md` 会直接带上发布工作流。 |
| 中文 release notes 默认化 | GitHub Release notes 默认中文，避免公开发布说明语言不一致。 |

### v0.3.2 更新重点

| 更新 | 作用 |
| --- | --- |
| 同步最新 grill 技能 | 仓库内直接包含 `grilling`、`grill-with-docs`、`domain-modeling` 和 legacy `grill-me`。 |
| DXM project-grill 路由更新 | `/dxm` 会按空项目、已有文档/代码、小脚本分别走当前 grill skill 组合。 |
| `domain-modeling` 接入 | 已有代码/文档场景可同步维护 `CONTEXT.md`、`CONTEXT-MAP.md` 和 ADR。 |
| 兼容旧引用 | 旧的 `grill-me` 仍保留为 `grilling` 兼容别名，避免老文档失效。 |
...
| `--refresh-blocks` | 刷新 DXM 与 Trellis marker 管理块，保留 marker 外的人工维护内容。 |
| 半残 marker 显式失败 | 只有 `START` 没有 `END` 时直接报错，避免文档永久卡在坏状态。 |
| `--inventory-depth N` | 生成文件结构快照时可展开多层子目录，减少首次手工补结构。 |
| `--self-test` | 安装后的 skill 可直接跑内置冒烟检查，不依赖仓库测试目录。 |
| 不静默覆盖 | 默认只补缺失文件，不静默覆盖已有项目文档。 |
| 敏感文件降噪 | 避免把 `token_utils.py`、`password-reset.tsx`、`secret-management.md` 这类源码/文档误标为敏感数据。 |
...
- 在任何 DXM/Trellis marker 写入前预检已有目标文件必须是 UTF-8，且 marker 必须成对，避免失败后留下半写入或坏块静默成功。
...
| `开发者AI开发与PR提交流程.md` | Git、分支、PR、GitHub CLI、合并授权、发布 / Release 与 Latest 核验流程 |

默认只创建缺失文件，避免覆盖人工长期维护的内容。需要升级 DXM 或 Trellis 管理块时，使用 `--refresh-blocks`，脚本只会刷新 marker 内的生成内容。
