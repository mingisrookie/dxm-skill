...
| 更新 | 作用 |
| --- | --- |
| 本仓自用 DXM | 仓库自身新增 `AGENTS.md` 和四份长期文档，后续维护也按 DXM 规则执行。 |
| 三处契约防漂移 | 新增 `tests/test_doc_sync.py`，约束 `SKILL.md`、模板和 `scaffold_dxm.py` 的文件清单、grill 路由和 Trellis 命令一致。 |
| `/dxm` 分支更明确 | 只读请求会停在检查；`scaffold only` 会跳过 grill 并继续补模板。 |
| Trellis 命令对齐 | 文档明确 `trellis init --codex -u <developer> -y --skip-existing`，和脚本实现一致。 |
| `--force` 风险说明 | 明确它会覆盖已有 DXM 目标文件，只有用户接受人工内容丢失风险时才用。 |
| 链路文档补实 | `项目完整链路说明.md` 不再是模板占位，改为记录本仓真实技能触发 → 脚手架 → 模板 → 测试/CI 流程。 |
