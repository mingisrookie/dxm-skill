...
> 项目：`dxm`
> 根目录：当前仓库根目录；执行前根据真实 Git 工作区确认。
> 初始化日期：`2026-07-09`

仅在本次任务涉及 Git、PR 或发布时读取并执行适用部分；普通文件修改不启动整套发布流程。结论必须来自真实操作和验证。



## DXM 文档维护规则

- 本文由维护者按实际项目要求更新，保留已有项目专属规则和人工内容。
...
下列示例使用 GitHub CLI。执行前确认所选工具可用、身份和权限正确；也可使用经授权且具备等效读写核验能力的 GitHub 连接器，不为遵循示例强制安装 CLI。
...
如果没有可用且获授权的工具、登录到错误账号或权限不足，停止受影响的远端操作并说明原因，不准假装已经完成。
...
2. 发起 PR 前必须确认目标分支策略；按项目文档、用户指定目标和实际远端默认分支判断，不因存在名为 `dev` 的分支就自动选它；仍有实质歧义时再询问。
